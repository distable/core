...
from src_plugins.sd1111_plugin import devices, prompt_parser, sd_paths
...
def create_sampler(id, model):
    for s in samplers:
        if id == s.name or id in s.aliases:
            ...
            src_plugins.sd1111_plugin.sd_paths.config = s
...
def single_sample_to_image(sample):
    x_sample = src_plugins.sd1111_plugin.SDPlugin.decode_first_stage(src_plugins.sd1111_plugin.SDState.sdmodel, sample.unsqueeze(0))[0]
...
class CFGDenoiser(torch.nn.Module):
    ...
    def forward(self, x, sigma, uncond, cond, cond_scale, image_cond):
        ...
        if tensor.shape[1] == uncond.shape[1]:
            ...
            if src_plugins.sd1111_plugin.SDState.batch_cond_uncond:
            ...
            x_out = torch.zeros_like(x_in)
            batch_size = batch_size * 2 if src_plugins.sd1111_plugin.SDState.batch_cond_uncond else batch_size
